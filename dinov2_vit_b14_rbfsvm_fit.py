"""
DINOv2 ViT-B/14 + RBF SVM (无数据泄露)
特征: register tokens 全局平均池化 + L2 归一化
...
CACHE_DIR = "./dinov2_vitb_cache"
OUTPUT_DIR = "./dinov2_vitb_rbf_output"
CKPT_DIR   = "./dinov2_vitb_rbf_ckpt"
...
INPUT_SIZE = 224
N_AUG = 30
C_VALUES = [0.1, 0.3, 0.5, 0.8, 1.0, 2.0, 3.0, 5.0, 8.0, 10.0]
N_RUNS = 3
RANDOM_SEED = 42
...
# ==================== 加载 DINOv2 ViT-B/14 ====================
print("加载 DINOv2 ViT-B/14...")
try:
    model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14')
except Exception as e:
    print(f"torch.hub 加载失败 ({e})，尝试使用 timm...")
    import timm
...
def single_run(run_id, orig_feats, orig_labels, orig_paths, generate_plots=False):
    ...
    save_ckpt(run_id, {
        ...
        'mean_f1s': mean_f1s  # 保存以便绘制误差棒曲线
    })

    # ===== 图表与详细表格输出（仅第 1 次运行） =====
    if generate_plots:
        prefix = f"DINOv2_ViTB_Run{run_id}"

        # --- C 值曲线 ---
        plt.figure(figsize=(8, 5))
        plt.plot([str(c) for c in C_VALUES], mean_f1s, marker='o', color='#2ecc71', linewidth=2, markersize=8)
...
print("\n" + "="*60)
print("汇总: DINOv2 ViT-B/14 + RBF SVM")
print("="*60)
...
summary_df = pd.DataFrame([{
    'Method': 'DINOv2 ViT-B/14 + RBF SVM',
...
summary_df.to_csv(os.path.join(OUTPUT_DIR, 'DINOv2_ViTB_RBF_SVM_summary.csv'), index=False)
print(f"汇总结果已保存到 {OUTPUT_DIR}/DINOv2_ViTB_RBF_SVM_summary.csv")
...
if len(all_mean_f1s) == N_RUNS:
    ...
    plt.figure(figsize=(8, 5))
    plt.errorbar(range(len(C_VALUES)), mean_per_c, yerr=std_per_c,
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'DINOv2_ViTB_C_curve_errorbar.png'), dpi=150)
    plt.close()
    print(f"带误差棒的 C 值曲线已保存到 {OUTPUT_DIR}/DINOv2_ViTB_C_curve_errorbar.png")
else:
