...
CACHE_DIR  = "./eva02_svm_cache"
OUTPUT_DIR = "./eva02_svm_output"
CKPT_DIR   = "./eva02_svm_ckpt"
...
print(f"设备: {DEVICE}")
print("EVA02-L/14 + 图像空间 MixUp + 高斯噪声 + 随机擦除 + RBF SVM")
...
# ==================== 加载模型 ====================
print("加载 EVA02-L/14...")
model = timm.create_model('eva02_large_patch14_448', pretrained=True, num_classes=0)
...
def single_run(run_id, orig_feats, orig_labels, orig_paths, generate_plots=False):
    ...
    # ===== 图表与详细表格输出（仅第 1 次运行） =====
    if generate_plots:
        prefix = f"EVA02_L_SVM_Run{run_id}"

        # --- C 值曲线 ---
        plt.figure(figsize=(8, 5))
        plt.plot([str(c) for c in SVM_C_VALUES], mean_f1s, marker='o', color='#e74c3c', linewidth=2, markersize=8)
...
summary_df.to_csv(os.path.join(OUTPUT_DIR, 'EVA02_L_SVM_summary.csv'), index=False)
...
if len(all_mean_f1s) == N_RUNS:
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'EVA02_L_SVM_C_curve_errorbar.png'), dpi=150)
    plt.close()
